...
# Title: VideoTransmitter_grc
# Author: StarLink
# Copyright: StarLink
# Description: Video streaming - Transmitter side
...
import VideoTransmitter_epy_block_0 as epy_block_0  # embedded python block
import sip
...
def snipfcn_snippet_0(self):
    ...
    if __name__ == "__main__":
        ...
        cmd = r'vlc dshow:///dev/video0 --sout="#transcode{vcodec=mp2v,vb=200,scale=0.5,acodec=none,scodec=none}:duplicate{dst=file{dst=./im.ts,no-overwrite},dst=display}" --sout-all --sout-keep'
...
class VideoTransmitter(gr.top_block, Qt.QWidget):

    def __init__(self, MTU=1500):
        gr.top_block.__init__(self, "VideoTransmitter_grc", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("VideoTransmitter_grc")
        ...
        self.settings = Qt.QSettings("GNU Radio", "VideoTransmitter")
        ...
        for c in range(1, 2):
            self.controls_grid_layout_1.setColumnStretch(c, 1)
        self.epy_block_0 = epy_block_0.file_reader(file_path="im.ts", n_bytes=1000)
...
def main(top_block_cls=VideoTransmitter, options=None):
    if options is None:
